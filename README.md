...
**SNR-based add-noise degradation**: Add noise directly to clean speech using either generated noise (`white`, `colored`/LTASS-shaped) or an existing noise file at a target SNR. Babble is file-backed only (not generated by `add_noise`). When `noise_type=auto`, files named like `bab-t8.wav` are auto-recognized as multitalker babble sources.
...
my-project/
  ...
  metadata/
    ...
    matrices/           ← saved matrix CSVs grouped by pipeline
```
...
- In `add_noise`, if a supplied noise waveform/file is longer than speech, a random equal-length segment is selected before SNR mixing
...
Generated matrix CSVs are stored per pipeline:

```text
<project>/metadata/matrices/<pipeline_name>/<pipeline_name>.matrix.csv
```

In the GUI Matrix tab, you can run a matrix from a saved CSV (browse/select), upload a CSV, or provide a direct path.
